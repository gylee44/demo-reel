...
const dir = resolve('fixtures/narration');
mkdirSync(dir, { recursive: true });
const scratch = resolve('output/audio-source');
mkdirSync(scratch, { recursive: true });
const rate = Number(process.env.FIXTURE_RATE ?? 145);
...
for (const [id, text] of Object.entries(NARRATIONS)) {
  ...
  const metadata = JSON.parse(
    execFileSync('ffprobe', ['-v', 'error', '-show_format', '-of', 'json', out], {
      encoding: 'utf8',
    }),
  );
  manifest[id] = {
    file: `${id}.m4a`,
    textHash: hash(text),
    durationMs: Math.ceil(Number(metadata.format.duration) * 1000),
    cues,
    source: {
      voice: 'macOS Yuna',
      rate,
      purpose: 'Fixed PoC fixture; not a runtime TTS integration',
    },
  };
