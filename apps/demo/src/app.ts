...
export async function createDemoApp(db: Database, cfg: Config) {
  await db.pool.query(
    `CREATE TABLE IF NOT EXISTS dr_demo_tasks(id text PRIMARY KEY,title text UNIQUE NOT NULL,description text NOT NULL,done boolean NOT NULL DEFAULT false,created_at timestamptz DEFAULT now())`,
  );
  ...
  app.addHook('onRequest', async (req, reply) => {
    ...
    if (!token?.valid || !token.value || Number(token.value.split(':')[1]) < Date.now())
  ...
  app.post('/demo-api/login', async (req, reply) => {
    ...
    if (
      !b.success ||
      b.data.username !== DEMO_CREDENTIALS.username ||
      b.data.password !== DEMO_CREDENTIALS.password
    )
      return reply.code(401).send({ message: '테스트 계정 정보를 확인해 주세요.' });
    reply.setCookie('demo_session', `demo:${Date.now() + 3600000}`, {
      signed: true,
      httpOnly: true,
      sameSite: 'strict',
      path: '/',
  ...
  app.post('/demo-api/tasks', async (req, reply) => {
    ...
    const id = randomUUID();
    try {
      await db.pool.query('INSERT INTO dr_demo_tasks(id,title,description) VALUES($1,$2,$3)', [
        id,
        b.data.title,
        b.data.description,
      ]);
      ...
      if (e.code === '23505')
        return reply
          .code(409)
          .send({ message: '같은 제목의 업무가 이미 있습니다. 중복 생성하지 않았습니다.' });
      throw e;
    }
    return reply.code(201).send({ id, ...b.data, done: false });
  });
  app.post('/demo-api/tasks/:id/complete', async (req, reply) => {
    const r = await db.pool.query('UPDATE dr_demo_tasks SET done=true WHERE id=$1 RETURNING *', [
      (req.params as any).id,
    ]);
    if (!r.rowCount) return reply.code(404).send({ message: '업무를 찾을 수 없습니다.' });
    return r.rows[0];
  });
  ...
  app.get('/app.js', async (_req, reply) =>
    reply
      ...
      .send(
        `window.__drBrokenOverview=${brokenOverview};\n` +
          (await readFile(fileURLToPath(new URL('../public/app.js', import.meta.url)), 'utf8')),
      ),
  );
  app.get('/style.css', async (_req, reply) =>
    reply
      .type('text/css')
      .send(await readFile(fileURLToPath(new URL('../public/style.css', import.meta.url)), 'utf8')),
  );
  for (const path of [
    '/',
    '/login',
    ...
    app.get(path, async (_req, reply) =>
      reply
        .type('text/html')
        .send(
          await readFile(fileURLToPath(new URL('../public/index.html', import.meta.url)), 'utf8'),
        ),
    );
  return app;
}
