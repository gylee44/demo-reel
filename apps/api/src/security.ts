...
export function decrypt<T>(value: string, key: string): T {
  const data = Buffer.from(value, 'base64'); const decipher = createDecipheriv('aes-256-gcm', Buffer.from(key, 'hex'), data.subarray(0,12));
...
export function validatePocTarget(value: string, allowed: string) {
  let u: URL; try { u = new URL(value); } catch { throw new AppError('URL_BLOCKED', '올바른 URL을 입력해 주세요.'); }
  if (!['http:', 'https:'].includes(u.protocol) || u.username || u.password || u.origin !== allowed) throw new AppError('URL_BLOCKED', '현재 PoC는 준비된 테스트 앱만 연결할 수 있습니다.');
  return u;
}
