...
export function affectedScenes(plan: Plan, selected: string[]): string[] {
  ...
  for (const scene of plan.scenes) {
    if (
      scene.dependsOn.some((id) => affected.has(id)) ||
      [...scene.effects.reads, ...scene.effects.writes].some((k) => changed.has(k))
    )
...
export function sceneDuration(scene: Scene, audioMs: number, actionEndMs: number): number {
  if (![audioMs, actionEndMs].every((v) => Number.isFinite(v) && v >= 0))
...
export function mayRepeat(
  completed: boolean,
  safe: boolean,
): 'already_completed' | 'repeat' | 'needs_action' {
